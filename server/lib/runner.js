...
import { checkLinkedInAuth, getDefaultBrowserSession } from "./browserSession.js";
...
import {
  appendAudit as appendAuditTo,
  createRunStore,
  findLatestResumableRun as findLatestResumableRunIn,
  findSentActionMatches as findSentActionMatchesIn,
  listRuns as listRunsIn,
  loadRun as loadRunFrom,
  readAudit as readAuditFrom,
  recoverInterruptedRuns as recoverInterruptedRunsIn,
  saveRun as saveRunTo
} from "./runStore.js";
import { checkSafetyGate, randomizedDelayMs } from "./safetyPolicy.js";

/**
 * One runner = one LinkedIn profile: its own Chrome session, its own runs folder,
 * its own active run, wake handle and auth cache. The body below is the
 * single-profile runner unchanged — the state it used to keep in module scope now
 * lives in this closure, so a second profile gets its own identical copy and the
 * two never touch each other.
 */
export function createProfileRunner({ profileId = "default", browser, store } = {}) {
  // Without an explicit browser this is the original single-profile runner, so it
  // must drive the very same Chrome session the /api/chrome endpoints drive.
  const runnerBrowser = browser ?? getDefaultBrowserSession();
  const runStore = store ?? createRunStore();

  const { attach, closeTab, launch, listTabs, openTab } = runnerBrowser;
  const saveRun = (run) => saveRunTo(run, runStore);
  const loadRun = (runId) => loadRunFrom(runId, runStore);
  const appendAudit = (runId, entry) => appendAuditTo(runId, entry, runStore);
  const readAudit = (runId) => readAuditFrom(runId, runStore);
  const listRuns = () => listRunsIn(runStore);
  const recoverInterruptedRuns = (now) => recoverInterruptedRunsIn(runStore, now);
  const findLatestResumableRun = () => findLatestResumableRunIn(runStore);
  const findSentActionMatches = (snapshot) => findSentActionMatchesIn(snapshot, runStore);
  ...
  async function startCampaignRun(snapshot) {
  ...
  async function getCampaignRun(runId) {
    const run = await loadRun(runId);
    return decorateRun(run);
  }

  async function getActiveCampaignRun() {
    if (activeRunId === null) {
  ...
  async function startCampaignBatch(snapshots) {
  ...
  async function listCampaignRuns(profileId) {
  ...
  async function checkCampaignReplies(profileId, { force = false } = {}) {
    if (!profileId) {
  ...
  async function stopCampaignRun(runId) {
    const run = await loadRun(runId);
  ...
  async function pauseCampaignRun(runId) {
    const run = await loadRun(runId);
    if ([runStates.COMPLETED, runStates.FAILED, runStates.STOPPED].includes(run.state)) {
      return { ok: true, run: decorateRun(run) };
    }
  ...
  async function resumeCampaignRun(runId, proposedActions = []) {
...
/**
 * The default runner keeps the original single-profile entry points working
 * exactly as before: same runs folder, same Chrome session, same behaviour.
 */
const defaultRunner = createProfileRunner();

export const initializeRunner = (...args) => defaultRunner.initializeRunner(...args);
export const startCampaignRun = (...args) => defaultRunner.startCampaignRun(...args);
export const getCampaignRun = (...args) => defaultRunner.getCampaignRun(...args);
export const getActiveCampaignRun = (...args) => defaultRunner.getActiveCampaignRun(...args);
export const startCampaignBatch = (...args) => defaultRunner.startCampaignBatch(...args);
export const listCampaignRuns = (...args) => defaultRunner.listCampaignRuns(...args);
export const checkCampaignReplies = (...args) => defaultRunner.checkCampaignReplies(...args);
export const stopCampaignRun = (...args) => defaultRunner.stopCampaignRun(...args);
export const pauseCampaignRun = (...args) => defaultRunner.pauseCampaignRun(...args);
export const resumeCampaignRun = (...args) => defaultRunner.resumeCampaignRun(...args);
export const retryCampaignRun = (...args) => defaultRunner.retryCampaignRun(...args);
